...
namespace DevCodeArchitect.Utilities;
    ...
    public class Pagination
    {
            ...
            if (Links.Count > 0)
            {
                ...
                foreach (int Item in Links)
                {
                    ...
                    else
                    {
                        if (isFilter)
                        {
                            LinkURL = UtilityHelper.Add_pagenumber(FilterPaginationUrl, Item.ToString());
                        }
                        else
                        {
                            LinkURL = UtilityHelper.Add_pagenumber(DefaultPaginationUrl, Item.ToString());
                        }
                    }
                    string _css = "";
                    if (Item == PageNumber)
            ...
            bool ShowLast)
        {
            ...
            if (isFilter)
            {
                LastNavigationUrl = UtilityHelper.Add_pagenumber(FilterPaginationUrl, TotalPages.ToString());
                NextNavigationUrl = UtilityHelper.Add_pagenumber(FilterPaginationUrl, _nextpage.ToString());
            }
            else
            {
                LastNavigationUrl = UtilityHelper.Add_pagenumber(DefaultPaginationUrl, TotalPages.ToString());
                NextNavigationUrl = UtilityHelper.Add_pagenumber(DefaultPaginationUrl, _nextpage.ToString());
            }
