...
public class PaginationUtil
{
    ...
    public static string SetDefaultUrl()
    {
        var str = new StringBuilder();
        if (SiteConfigs.HttpContextAccessor != null && SiteConfigs.HttpContextAccessor.HttpContext != null)
        {
            var _context = SiteConfigs.HttpContextAccessor.HttpContext.Request;
            str.Append(_context.Path);
            var _query = _context.Query;
            if (_query != null && _query.Count > 0)
            {
                ...
                {
                    if (param.Key != "order" && param.Key != "filter")
                    {
                        if (index > 0)
                            str.Append("&");
    ...
    public static string PaginationUrl(string culture)
    {
        return culture + "/" + SetDefaultUrl() + "/p-[p]";
    }
}
