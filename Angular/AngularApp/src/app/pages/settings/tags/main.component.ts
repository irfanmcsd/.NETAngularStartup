...
import { TagService } from '../../../store_v2/tags/tags.services';
...
import {
  ...
  selectTagsLoading,
  selectRecords
} from '../../../store_v2/tags/tags.reducer';
...
export class MainTagsComponent implements OnInit, OnDestroy {
  ...
  data$: Observable<TagModel[]> = this.store.select(selectAllTags);
  records$: Observable<number> = this.store.select(selectRecords);

  // Component state
  navQuery: ITAG = { ...TAG_QUERY_OBJECT }; // Navigation query parameters
  isAuthenticated: boolean = false;
  User: UserModel = { ...Initial_User_Entity };
  selectedDocument: TagModel = { ...Initial_Tag_Entity };
  ...
  private initializeManageListings(): void {
    this.store.dispatch(loadTags({ Query: this.navQuery }));
  }
  ...
  private loadDocument(id: number): void {
    ...
    this.tagService.GetInfo({ ...TAG_QUERY_OBJECT, id }).pipe(
      takeUntil(this.destroy$)
    ).subscribe({
   ...
   */
  private handleDocumentResponse(record: TagModel): void {
   ...
   */
  deleteRecord(Record: TagModel, event: Event): void {
    this.openConfirmationDialog(
      'Delete Record',
      'Are you sure you want to delete selected record',
      () => {
        this.store.dispatch(actionTags({ 
          Entities: [Record], 
          actionstatus: 'delete' 
        }));
      },
